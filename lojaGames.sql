...
SELECT * FROM tb_produtos INNER JOIN tb_categorias ON td_produtos.categoria_id = tb_categorias.id WHERE td_produtos.categoria_id =3; 
-- ou where td_produtos.categoria_id = "Aventura";

SELECT * FROM tb_produtos INNER JOIN tb_usuarios ON td_produtos.usuarios_id = tb_usuarios.id ORDER BY tb_produtos.nome;

-- relacionar trÊs tabelas: começa com a tabela que tem as chaves estrangeiras e faz um inner join com cada tabela que ela tem relação
